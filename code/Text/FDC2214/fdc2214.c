...
{					     
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1;//端口配置 PB0接FDC2214模块ADD，GND接FDC2214模块SD
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		//推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		//IO口速度为50MHz
  GPIO_Init(GPIOB, &GPIO_InitStructure);					    //根据设定参数初始化GPIO 
  GPIO_ResetBits(GPIOB,GPIO_Pin_0|GPIO_Pin_1);				  //PB0【SD】，PB1【ADD】输出低	
...
unsigned long FilteredFCD2214ReadCH(unsigned char index)//对读取值除去最小最大取平均滤波
{
	unsigned char i=0;
	unsigned long temp[6] = {0},longSum=0,tempMax=0,tempMin=0;

	for(i=0;i<=5;i++)	
	{
		temp[i]=(FCD2214_ReadCH(index) | 0x000003FF) & 0xFFFFFC00;//取高18位
		delay_ms(1);		
	}
	longSum=temp[0];
	tempMax=temp[0];
	...
	for(i=1;i<=5;i++)
	{
		if(tempMax<temp[i])tempMax=temp[i];//记录最?值
		if(tempMin>temp[i])tempMin=temp[i];//记录最小值
		longSum=temp[i]+longSum;//累加
		
	}

	return((longSum-tempMax-tempMin)/4);//6个值，去除最大值，去除最小值，其余4个值取平均
}
...
//		Set_FDC2214(CONFIG,0x15,0x01);//激活模式，使用外接40M有源晶振做参考频率		
	}
	else return 1;
